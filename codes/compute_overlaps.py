#!/usr/bin/env python3
"""
For each alpha_* directory and each realisation r:
  - reads spins/spins_r{r}.csv        (one row per sweep; first column is the sweep index)
  - reads patterns/patterns_N{N}_r{r}.csv  (one row per pattern)
  - computes the bit-safe overlap of every sweep with every pattern
  - saves a DataFrame to overlaps/overlaps_alpha{a}_r{r}.csv
...
# ============================================================
# DETECT N FROM PATTERN FILENAMES
# ============================================================
...
for alpha_dir in sorted(root_dir.glob("alpha_*"),
    ...
    # build map: realisation index -> pattern file
    pattern_map = {}
    ...
    for spin_file in sorted(spins_dir.glob("*.csv")):
        ...
        # first column of spins file is the sweep index
        ...
        # compute overlap matrix (n_sweeps x n_patterns)
        M = np.empty((n_sweeps, n_patterns), dtype=float)

        for sweep_idx, spin_row in spins_df[block_cols].iterrows():
            for pat_idx, pat_row in pattern_df.iterrows():
                M[sweep_idx, pat_idx] = overlap_binary(spin_row, pat_row, N)
