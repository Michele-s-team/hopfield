...
def compute_overlap(spins_a, spins_b):
    """Overlap m = (1/N) sum_i s_i * xi_i, with Z2 symmetry (abs)."""
    return float(np.mean(spins_a.astype(np.int16) * spins_b.astype(np.int16)))
...
if __name__ == "__main__":
    ...
    # ── Decode all patterns ───────────────────────────────────────────────────
    all_patterns = []
    for _, prow in pattern_df.iterrows():
        all_patterns.append(decode_row_to_spins(prow, N, n_blocks))
    P = len(all_patterns)
    print(f"Number of patterns P = {P}")

    # ── Decode all spin configurations ───────────────────────────────────────
    ...
    # ── Find the best-matching pattern for the final configuration ───────────
    final_spins = spin_frames[-1].ravel()

    best_mu      = -1
    best_overlap = -1.0
    perfect_match = False

    for mu, pat in enumerate(all_patterns):
        m = compute_overlap(final_spins, pat)
        if abs(m) > best_overlap:
            best_overlap = abs(m)
            best_mu      = mu
        if np.array_equal(final_spins, pat) or np.array_equal(final_spins, -pat):
            perfect_match = True
            best_mu       = mu
            best_overlap  = abs(m)
            break

    best_pattern_2d = all_patterns[best_mu].reshape(L, L)

    if perfect_match:
        match_str = f"perfect match"
    else:
        match_str = f"best match: |m^\mu|={best_overlap:.4f}"

    print(f"\nFinal config: {match_str}")

    # ── Plot ──────────────────────────────────────────────────────────────────
    fig, (ax_left, ax_right) = plt.subplots(1, 2, figsize=(10, 5))

    im_left = ax_left.imshow(
        spin_frames[0], cmap="binary", vmin=-1, vmax=1, interpolation="nearest"
    )
    ...
    im_right = ax_right.imshow(
        best_pattern_2d, cmap="binary", vmin=-1, vmax=1, interpolation="nearest"
    )
    ax_right.set_title(f"Pattern μ={best_mu}  ({match_str})")
    ax_right.set_xticks([])
    ax_right.set_yticks([])
    ...
    anim = animation.FuncAnimation(
        fig, update, frames=n_frames, interval=200, blit=False, repeat=True
    )
