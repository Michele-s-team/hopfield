...
def extract_N_beta_from_spins_file(path: Path):
    name = path.name
...
if __name__ == "__main__":
    ...
    spins_dir = first_alpha / "spins"
    spins_files = list(spins_dir.glob("spins*_r*.csv"))

    if not spins_files:
        raise RuntimeError("No spins files found to infer N/beta")

    N_label, beta_label = extract_N_beta_from_spins_file(spins_files[0])

    df_export.to_csv(
        f"../save_old/25_June/max_overlap_vs_sweeps_N{N_label}_beta{beta_label}.csv",
        index=False
    )
    ...
    n_alpha = 2   # index dans ALL_ALPHA_DIRS (0-based)
    ...
    ofile = overlaps_dir / f"overlaps_N1024_beta5.0_r{r_target}.csv" #TO BE CHANGED
    ...
    spins_files = list(spins_dir.glob(f"spins*_r{r_target}.csv"))

    if not spins_files:
        raise RuntimeError(f"No spins file found for r={r_target}")

    N_label, _ = extract_N_beta_from_spins_file(spins_files[0])
    ...
    if not spins_files:
        raise RuntimeError("No spins files found")

    N_label, beta_label = extract_N_beta_from_spins_file(spins_files[0])

    plt.figure(figsize=(9, 5))

    ofiles = sorted(overlaps_dir.glob("overlaps*.csv"))
    n_real   = len(ofiles)
