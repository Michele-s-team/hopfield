...
\foreach \i/\label in {0/0, 1/1, 2/2} {
    ...
    \node[font=\small] at (\i*\step+0.3, 0.3) {$b_{\label}$};
}
% ...
...
64-bit word (\Bits) & $S_i \in \{0,1\}^{64}$ \\
...
% Left label
\node[left] at ({-0.25},{-1.5*\rowsep+0.5*\boxsize}) {$Z=$};
...
\end{tikzpicture}
\caption{An \UnsignedInt encoding 64 parallel unsigned integers. Each row is a \Bits associated with a power of 2. The value of each integer is obtained by reading the \UnsignedInt along the corresponding column. For instance, the fourth integer stored in $Z$ is $Z_4=\sum\limits_{i=0}^l b_4^i\, 2^i$.}
...
\myindent The bitwise implementation is a structural optimization of the representation of data. On top of this structural optimization can be added a physical one that is that all replica of the system can share the same sequence of random numbers. In this way, the evolutions the many replicas of the system are completely parallelized. This optimization turns out to be central, as the generation of the random number is often times the bottleneck of the simulation, as it typically requires non-trivial operations such as multiplications and is called in the hot loop of the algorithm. A a schematic view of the shared random number is shown in Figure~\ref{fig:shared_rng}
...
\foreach \i/\label in {0/0, 1/1, 2/2} {
    \draw (\i*\step, 0) rectangle (\i*\step+\boxw, 0.6);
    \node[font=\small] at (\i*\step+0.3, 0.3) {$b_{\label}^1$};
}
