...
\end{equation}
The bitwise implementation of this condition requires that all quantities are positive. We thus define the shifted extensive overlaps $ \tilde{M}^\mu = M^\mu + N$, $\tilde{M}^\mu \in[0, 2N]$. Then, with $S_k \xi_k^\mu = 2 c_k^\mu -1$ with $c_k^\mu = S_k \odot \xi_k^\mu$,
\begin{align*}
...
\end{equation}
and no summation over the non-neighbors is required.
...
\caption{Evolution of the reconstruction error with the storage load. For this diagram, the configuration is initialized on one of the stored patterns.}
...
\myindent We present in \Cref{fig:speedup_hopfield} the performance analysis of the code. Several values of $\alpha$ and lattice sizes have been tested for an evolution of $2^{10}$ sweeps each.
